...
function MessageCard({ msg }: { msg: DebateMessage }) {
  ...
  return (
    ...
    >
      ...
      >
        <AIAvatar name={msg.ai_model_name} size={20} />
        ...
        {open ? <ChevronUp size={12} className="text-gray-500 sm:w-3.5 sm:h-3.5" /> : <ChevronDown size={12} className="text-gray-500 sm:w-3.5 sm:h-3.5" />}
      </div>
        ...
        <div className="mt-2 sm:mt-3 space-y-2 sm:space-y-3">
          ...
          {parsed.reasoning && (
            <div className="bg-black/20 rounded-lg p-2 sm:p-3">
              <div className="text-[10px] sm:text-xs text-blue-400 font-medium mb-1.5 sm:mb-2">💭 思考过程 / Reasoning</div>
          ...
          {msg.decision && (
            <div className="bg-black/20 rounded-lg p-2 sm:p-3">
              <div className="text-[10px] sm:text-xs text-green-400 font-medium mb-1.5 sm:mb-2">📊 交易决策 / Decision</div>
...
function VoteCard({ vote }: { vote: { ai_model_name: string; action: string; symbol?: string; confidence: number; leverage?: number; position_pct?: number; stop_loss_pct?: number; take_profit_pct?: number; reasoning: string } }) {
  ...
  return (
    <div className="bg-[#1a1f2e] rounded-lg sm:rounded-xl p-2 sm:p-4 border border-white/10 hover:border-white/20 transition-all">
      <div className="flex items-center justify-between mb-2 sm:mb-3 flex-wrap gap-1.5 sm:gap-2">
        <div className="flex items-center gap-1.5 sm:gap-2 min-w-0 flex-1">
          <AIAvatar name={vote.ai_model_name} size={24} />
          <div className="min-w-0 flex-1">
            <span className="text-xs sm:text-sm text-white font-semibold block truncate">{vote.ai_model_name}</span>
            {vote.symbol && <span className="text-[10px] sm:text-xs text-gray-400 truncate">{vote.symbol}</span>}
          </div>
        </div>
        <span className={`flex items-center gap-0.5 sm:gap-1 px-1.5 sm:px-2.5 py-0.5 sm:py-1 rounded-lg text-[10px] sm:text-xs font-bold ${a.bg} ${a.color}`}>
          ...
          <span className="sm:hidden">{vote.action.replace('_', ' ').toUpperCase().slice(0, 4)}</span>
        </span>
      </div>
      ...
      <div className="grid grid-cols-2 gap-x-2 sm:gap-x-4 gap-y-0.5 sm:gap-y-1 text-[10px] sm:text-sm">
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-2 sm:p-4">
      <div className="bg-[#1a1d24] rounded-xl w-full max-w-md p-3 sm:p-4 border border-white/10 max-h-[90vh] overflow-y-auto">
        <div className="flex justify-between items-center mb-3 sm:mb-4">
          ...
          <button onClick={onClose}><X size={18} className="text-gray-400 sm:w-5 sm:h-5" /></button>
        </div>

        <div className="space-y-2 sm:space-y-3">
          <input
            value={name} onChange={e => setName(e.target.value)}
            placeholder={t('debateName', language)} className="w-full px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg bg-white/5 border border-white/10 text-white text-xs sm:text-sm"
          />

          {/* Strategy selector - moved up */}
          <select value={strategyId} onChange={e => setStrategyId(e.target.value)}
            className="w-full px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg bg-white/5 border border-white/10 text-white text-xs sm:text-sm">
          ...
          <div className="flex flex-col sm:flex-row gap-2">
            ...
            {isStaticWithCoins ? (
              <select value={symbol} onChange={e => setSymbol(e.target.value)}
                className="flex-1 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg bg-white/5 border border-white/10 text-white text-xs sm:text-sm">
                {staticCoins.map(coin => <option key={coin} value={coin}>{coin}</option>)}
              </select>
            ) : (
              <div className="flex-1 px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg bg-white/5 border border-white/10 text-gray-400 text-xs sm:text-sm">
                {language === 'zh' ? '根据策略规则自动选择' : 'Auto-selected by strategy'}
              </div>
            )}
            <select value={maxRounds} onChange={e => setMaxRounds(+e.target.value)}
              className="px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg bg-white/5 border border-white/10 text-white text-xs sm:text-sm">
        ...
        <div className="flex flex-col sm:flex-row gap-2 mt-3 sm:mt-4">
          ...
          <button onClick={submit} disabled={creating}
            ...
            {creating ? <Loader2 size={14} className="animate-spin mx-auto sm:w-4 sm:h-4" /> : t('create', language)}
          </button>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="h-full bg-[#0a0c10] flex flex-col lg:flex-row overflow-hidden">
      ...
      <div className="w-full lg:w-56 flex-shrink-0 bg-[#0d1017] border-b lg:border-b-0 lg:border-r border-white/5 flex flex-col max-h-[300px] lg:max-h-none">
          ...
          {debates?.map(d => (
            <div key={d.id} onClick={() => setSelectedId(d.id)}
              ...
              <div className="flex items-center gap-1.5 sm:gap-2">
                <span className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full flex-shrink-0 ${STATUS_COLOR[d.status]}`} />
                <span className="text-xs sm:text-sm text-white truncate flex-1 min-w-0">{d.name}</span>
              </div>
              <div className="text-[10px] sm:text-xs text-gray-500 mt-0.5 sm:mt-1 truncate">{d.symbol} · R{d.current_round}/{d.max_rounds}</div>
              {d.status === 'pending' && selectedId === d.id && (
                <div className="flex gap-1 mt-1 flex-wrap">
                  <button onClick={e => { e.stopPropagation(); onStart(d.id) }}
                    className="text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 bg-green-500/20 text-green-400 rounded">{t('start', language)}</button>
                  <button onClick={e => { e.stopPropagation(); onDelete(d.id) }}
                    className="text-[10px] sm:text-xs px-1.5 sm:px-2 py-0.5 bg-red-500/20 text-red-400 rounded">{t('delete', language)}</button>
                </div>
              )}
            </div>
          ))}
        ...
        <div className="hidden lg:flex flex-1 border-t border-white/5 mt-2 overflow-hidden flex-col">
              ...
              <div key={tr.trader_id}
                ...
                <div className="flex items-center gap-1.5 sm:gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-xs sm:text-sm text-white font-medium truncate">{tr.trader_name}</div>
                    <div className="text-[10px] sm:text-xs text-gray-500 truncate">{tr.ai_model}</div>
                  </div>
            ...
            {traders?.filter(tr => !tr.is_running).slice(0, 3).map(tr => (
              <div key={tr.trader_id} className="p-1.5 sm:p-2 rounded-lg bg-white/5 opacity-50">
                <div className="flex items-center gap-1.5 sm:gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="text-xs sm:text-sm text-white font-medium truncate">{tr.trader_name}</div>
                    <div className="text-[10px] sm:text-xs text-gray-500">{t('offline', language)}</div>
                  </div>
                </div>
              </div>
            ))}
      ...
      <div className="flex-1 flex flex-col min-w-0 overflow-hidden">
        {detail ? (
          <>
            {/* Header Bar - Compact */}
            <div className="px-2 sm:px-3 py-1.5 sm:py-2 border-b border-white/5 bg-[#0d1017]/50 flex flex-wrap items-center gap-2 sm:gap-3 flex-shrink-0">
              <span className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full flex-shrink-0 ${STATUS_COLOR[detail.status]}`} />
              <span className="text-sm sm:text-base font-bold text-white truncate flex-1 min-w-0">{detail.name}</span>
              <span className="text-xs sm:text-sm text-yellow-400 font-semibold">{detail.symbol}</span>
              {strategyName && <span className="text-[10px] sm:text-xs px-1 sm:px-1.5 py-0.5 bg-purple-500/20 text-purple-400 rounded truncate">{strategyName}</span>}
              <span className="text-[10px] sm:text-xs text-gray-500">R{detail.current_round}/{detail.max_rounds}</span>

              {/* Participants */}
              <div className="flex gap-0.5 sm:gap-1 ml-1 sm:ml-2 flex-wrap">
                {participants.map(p => {
                  const vote = votes.find(v => v.ai_model_id === p.ai_model_id)
                  const act = vote ? (ACT[vote.action] || ACT.wait) : null
                  return (
                    <div key={p.id} className="flex items-center gap-0.5 sm:gap-1 px-0.5 sm:px-1 py-0.5 rounded bg-white/5 text-[10px] sm:text-xs">
                      <AIAvatar name={p.ai_model_name} size={12} />
              ...
              {votes.length > 0 && (
                <div className="flex gap-0.5 sm:gap-1 flex-wrap">
                  {Object.entries(voteSum).map(([action, count]) => {
                    ...
                    return (
                      <div key={action} className={`flex items-center gap-0.5 sm:gap-1 px-1 sm:px-1.5 py-0.5 rounded ${cfg.bg} ${cfg.color} text-[10px] sm:text-xs font-semibold`}>
            ...
            <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
              {Object.keys(rounds).length === 0 ? (
                <div className="flex-1 flex flex-col items-center justify-center text-gray-500 p-4">
                  ...
                  <div className="text-sm sm:text-lg text-center">{detail.status === 'pending' ? t('clickToStart', language) : t('waitingAI', language)}</div>
                </div>
              ) : (
                <>
                  {/* Left - Rounds */}
                  <div className="flex-1 overflow-y-auto p-2 sm:p-4 border-b lg:border-b-0 lg:border-r border-white/5 min-w-0">
                    <div className="text-xs sm:text-sm text-gray-400 font-semibold mb-2 sm:mb-3 flex items-center gap-2">
                    ...
                    <div className="space-y-2 sm:space-y-3">
                      {Object.entries(rounds).map(([round, msgs]) => (
                        <div key={round} className="bg-white/5 rounded-lg sm:rounded-xl p-2 sm:p-3">
                  ...
                  {/* Right - Votes */}
                  {votes.length > 0 && (
                    <div className="w-full lg:w-[420px] flex-shrink-0 overflow-y-auto p-2 sm:p-4 bg-[#0d1017]/50 border-t lg:border-t-0 lg:border-l border-white/5">
                      <div className="text-xs sm:text-sm text-gray-400 font-semibold mb-2 sm:mb-3 flex items-center gap-2">
            ...
            {(decision || votes.length > 0) && (
              <div className="p-2 sm:p-3 border-t border-white/5 bg-gradient-to-r from-yellow-500/10 to-orange-500/10 flex flex-col sm:flex-row items-start sm:items-center gap-2 sm:gap-4 flex-shrink-0">
                <div className="flex items-center gap-1.5 sm:gap-2 flex-wrap">
                  <Trophy size={16} className="text-yellow-400 sm:w-5 sm:h-5" />
                  <span className="text-xs sm:text-sm text-gray-400">{t('consensus', language)}:</span>
                  {decision ? (
                    <>
                      {decision.symbol && <span className="text-xs sm:text-sm text-yellow-400 font-bold">{decision.symbol}</span>}
                      <span className={`flex items-center gap-0.5 sm:gap-1 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded font-bold text-xs sm:text-sm ${(ACT[decision.action] || ACT.wait).bg} ${(ACT[decision.action] || ACT.wait).color}`}>
                        {(ACT[decision.action] || ACT.wait).icon}
                        <span className="hidden sm:inline">{decision.action.replace('_', ' ').toUpperCase()}</span>
                        <span className="sm:hidden">{decision.action.replace('_', ' ').toUpperCase().slice(0, 4)}</span>
                      </span>
                    </>
                  ) : (
                    <span className="flex items-center gap-1 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded font-bold text-xs sm:text-sm bg-gray-500/20 text-gray-400">
                ...
                {decision && !decision.executed && (decision.action === 'open_long' || decision.action === 'open_short') && (
                  <button onClick={() => setExecId(detail.id)}
                    className="w-full sm:w-auto px-3 sm:px-4 py-1 sm:py-1.5 rounded-lg bg-yellow-500 text-black font-semibold text-xs sm:text-sm flex items-center justify-center gap-1">
      ...
      {execId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 p-2 sm:p-4">
          <div className="bg-[#1a1d24] rounded-xl w-full max-w-sm p-3 sm:p-4 border border-white/10">
            <h3 className="text-base sm:text-lg font-bold text-white mb-3 sm:mb-4 flex items-center gap-1.5 sm:gap-2">
              <Zap className="text-yellow-400 w-4 h-4 sm:w-5 sm:h-5" /> {t('executeTitle', language)}
            </h3>
            <select value={traderId} onChange={e => setTraderId(e.target.value)}
              className="w-full px-2 sm:px-3 py-1.5 sm:py-2 rounded-lg bg-white/5 border border-white/10 text-white text-xs sm:text-sm mb-2 sm:mb-3">
            ...
            <div className="flex flex-col sm:flex-row gap-2">
              ...
              <button onClick={onExecute} disabled={!traderId || executing || !traders?.find(tr => tr.trader_id === traderId)?.is_running}
                className="flex-1 py-1.5 sm:py-2 rounded-lg bg-yellow-500 text-black font-semibold text-xs sm:text-sm disabled:opacity-50">
                {executing ? <Loader2 size={14} className="animate-spin mx-auto sm:w-4 sm:h-4" /> : t('execute', language)}
              </button>
            </div>
          </div>
        </div>
