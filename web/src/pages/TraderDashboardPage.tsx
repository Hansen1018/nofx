...
export function TraderDashboardPage({
  ...
  const [copiedTraderId, setCopiedTraderId] = useState<boolean>(false)
  ...
  // Copy trader ID to clipboard
  const handleCopyTraderId = async () => {
    if (!selectedTrader?.trader_id) return
    try {
      await navigator.clipboard.writeText(selectedTrader.trader_id)
      setCopiedTraderId(true)
      setTimeout(() => setCopiedTraderId(false), 2000)
    } catch (err) {
      console.error('Failed to copy trader ID:', err)
    }
  }
    ...
    <DeepVoidBackground className="min-h-screen pb-12" disableAnimation>
      <div className="w-full px-4 md:px-8 relative z-10 pt-6">
        ...
        >
              ...
              <div className="flex flex-col min-w-0 flex-1">
                ...
                <div className="flex items-center gap-2 mt-0.5 group/trader-id">
                  ...
                  <button
                    ...
                    className="p-1 rounded hover:bg-white/10 transition-colors shrink-0 opacity-0 group-hover/trader-id:opacity-100"
                    title={
                      showFullTraderId
                        ? language === 'zh'
                          ? '隐藏完整ID'
                  ...
                  <button
                    type="button"
                    onClick={handleCopyTraderId}
                    className="p-1 rounded hover:bg-white/10 transition-colors shrink-0 opacity-0 group-hover/trader-id:opacity-100"
                    title={language === 'zh' ? '复制ID' : 'Copy ID'}
                  >
                    {copiedTraderId ? (
                      <Check className="w-3 h-3 text-nofx-green" />
                    ) : (
                      <Copy className="w-3 h-3 text-nofx-text-muted" />
                    )}
                  </button>
                </div>
              </div>
            ...
            <div className="flex items-center gap-3 flex-wrap">
              ...
              {exchanges && isPerpDex && (
                <div className="flex items-center gap-2 px-3 py-1.5 rounded-lg nofx-glass border border-nofx-gold/20">
                  {walletAddress ? (
                    <>
                      <span className="text-xs font-mono text-nofx-gold truncate max-w-[120px] sm:max-w-[200px]">
                        {showWalletAddress
                          ? walletAddress
          ...
          <div className="flex flex-col md:flex-row md:items-center gap-3 md:gap-6 text-sm text-nofx-text-muted font-mono pl-2">
            <span className="flex items-center gap-2 shrink-0">
              <span className="opacity-60">AI Model:</span>
              <span
                className="font-bold px-2 py-0.5 rounded text-xs tracking-wide shrink-0"
                style={{
                  ...
                  color: selectedTrader.ai_model.includes('qwen')
                    ? '#c084fc'
                    : '#60a5fa',
                  border: selectedTrader.ai_model.includes('qwen')
            ...
            <span className="flex items-center gap-2 shrink-0">
              ...
              <span className="text-nofx-text-main font-semibold truncate max-w-[200px] sm:max-w-[300px]">
            ...
            <span className="flex items-center gap-2 min-w-0 flex-1">
              <span className="opacity-60 shrink-0">Strategy:</span>
              <span
                className="text-nofx-gold font-semibold tracking-wide break-words min-w-0"
                title={selectedTrader.strategy_name || 'No Strategy'}
              >
              ...
              {decisions && decisions.length > 0 ? (
                decisions.map((decision, i) => (
                  <DecisionCard
                    ...
                    previousDecision={decisions[i - 1]}
                  />
                ))
              ) : (
