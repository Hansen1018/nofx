...
### 7. AI/LLM 消费者视角 E2E 可见性审计（BLOCKING）
⚠️ **AI/LLM 也是系统的"用户"** - 数据计算正确但 AI 看不到 = 功能无效！

**核心原则**：在 AI 驱动的系统中，AI/LLM 是数据的最终消费者之一。任何新增数据/字段都必须追踪到最终输出点（Prompt/Context），否则等于没实现。

#### 7.1 数据流完整性验证（MANDATORY）
对于每个新增/修改的数据字段，必须验证完整的数据流：

```
数据源 → 获取/计算 → 存储/缓存 → 格式化/序列化 → 最终输出（Prompt/API/UI）
```

**验证清单**：
- [ ] **数据获取**：数据能从源头正确获取（API/WebSocket/数据库/文件）
- [ ] **数据计算**：计算逻辑正确，结果存储到正确的字段
- [ ] **数据存储**：数据正确存储到内存/缓存/持久化层
- [ ] **格式化输出**：数据被包含在输出函数/序列化逻辑中
- [ ] **消费者可见**：最终消费者（AI/用户/下游系统）能获取并使用该数据

#### 7.2 输出点验证（MANDATORY）
**通用检查方法**：
1. **识别数据消费者**：这个数据是给谁用的？（AI/用户/下游服务）
2. **找到输出函数**：数据通过什么函数/方法传递给消费者？
3. **验证数据包含**：输出函数是否包含新增字段？

**验证方法**：
```bash
# 1. 找出新增字段的定义和赋值
grep -rn "NewFieldName" --include="*.go" --include="*.ts" --include="*.py"

# 2. 追踪到输出/序列化点
# 查找 JSON 序列化、字符串格式化、模板渲染等
grep -rn "NewFieldName" | grep -E "json|format|template|render|output|write|send"

# 3. 如果找不到输出点，该字段对消费者不可见！
```

#### 7.3 典型遗漏场景（高发问题）
| 场景 | 症状 | 检查点 |
|------|------|--------|
| **字段计算了但未输出** | 新字段有值，输出中没有 | 输出/序列化函数是否包含该字段 |
| **数据获取了但未传递** | 数据结构有字段，下游没收到 | 数据传递链是否完整 |
| **格式化了但格式错误** | 消费者收到数据但无法解析 | 输出格式是否符合消费者预期 |
| **订阅了但未处理** | 有数据流入，但未存储/处理 | 数据处理回调是否正确实现 |
| **存储了但未读取** | 数据在缓存中，但获取函数未返回 | getter 函数是否返回该字段 |
...
**数据给 AI/LLM 使用时**：
- [ ] 数据在上下文构建函数中被使用
- [ ] 数据正确传递给 AI/LLM Client
- [ ] Prompt/Context 中该数据位置和格式合理
- [ ] AI 能解析这个格式（结构化 vs 自然语言）
...
**数据给下游系统使用时**：
- [ ] 接口契约包含该字段
- [ ] 序列化格式正确（JSON/Protobuf/等）
- [ ] 下游系统能正确解析

#### 7.5 强制验证问题（必须回答）
对于任何数据相关的代码修改，审查者必须回答：

1. **这个数据是给谁用的？**
   - AI/LLM？用户界面？下游系统？内部逻辑？
...
#### 7.6 AI/LLM E2E 审查失败条件（BLOCKING）
...
**记住：对于任何系统，"代码能跑" ≠ "消费者能用"！数据必须完整流到最终输出才算功能完成。**

---

### 8. Web3 AI 交易系统安全审查（BLOCKING 级别）
