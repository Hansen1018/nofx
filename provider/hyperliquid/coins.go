...
// GetAllCoins returns all available Hyperliquid perp coins (package-level)
func GetAllCoins() ([]CoinInfo, error) {
	return GetProvider().GetAllCoins(context.Background())
}

// GetMainCoins returns top N coins by 24h volume (package-level)
func GetMainCoins(limit int) ([]CoinInfo, error) {
	return GetProvider().GetMainCoins(context.Background(), limit)
}
